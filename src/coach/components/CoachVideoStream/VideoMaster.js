...
async function joinSession(formValues) {
  ...
  if (!formValues.sendAudio || !formValues.sendVideo) {
    console.error(
      "[MASTER] Both Send Video and Send Audio checkboxes need to be checked for calling join session"
    );
    return;
  }
  ...
  try {
    ...
    console.error(
      "[MASTER] Storage Session is not configured for this channel"
    );
    return;
  }
  ...
  console.log(
    "[MASTER] Received webrtc endpoint: " + endpointsByProtocol.WEBRTC
  );
  ...
  const response = await fetch(endpointsByProtocol.WEBRTC + request.path, {
    ...
    .then(({ status, json, ok }) => {
      if (!ok) {
        console.log(
          "[MASTER] Error occured while calling join session: ",
          json
        );
      } else {
