...
const ProfileCaseNote = (props) => {
    ...
    useEffect(_ => {
        ...
        axios.post("https://d5btr8maz5.execute-api.ap-southeast-1.amazonaws.com/prod/casenote", JSON.stringify(data))
        .then(res => {
        ...
        const setSessionStorage = (page, firstItem) => {
            sessionStorage.setItem(page+"_CN", firstItem)
        }
    }, [])
    ...
    const prevPage = _ => {
        const data = {"id": {"id": {"S": sessionStorage.getItem(page-2+"_CN")}, "client": {"S": props.userid}}, "clientId": props.userid}
        if(page-1 == 1){
            data.id = null       
        }
        axios.post("https://d5btr8maz5.execute-api.ap-southeast-1.amazonaws.com/prod/casenote", JSON.stringify(data))
